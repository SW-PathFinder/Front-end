...
describe("초기화", () => {
  test("새로 생성된 보드는 출발지, 숨겨진 도착지가 설정되어 있어야 한다", () => {
    ...
    for (const destination of GameBoard.destinationCoordinates) {
      expect(board.getCard(...destination)).toBeInstanceOf(PathCard.DestHidden);
    }
  });

  test("보드는 startNewRound 함수를 통해 보드 상태를 라운드 시작시의 상태로 설정할 수 있다", () => {
    const board = new GameBoard();
    board.placeCard(1, 0, new PathCard.Way4());
    board.placeCard(0, 1, new PathCard.Way4());
...
describe("카드 가져오기", () => {
  ...
  test("보드에 있는 모든 카드들을 좌표와 함께 가져올 수 있다", () => {
    ...
    const card1 = new PathCard.Way4();
    const card2 = new PathCard.Block4();
...
describe("상태 불러오기", () => {
  test("정상적인 보드 상태를 불러올 수 있다", () => {
    const board = new GameBoard();
    board.import([[[1, 0], new PathCard.Block4()]]);
    expect(board.getCard(1, 0)).toBeInstanceOf(PathCard.Block4);

    // 도착지 카드를 알맞은 카드로 덮어쓸 수 있음
    const dest1Coord = GameBoard.destinationCoordinates[0];
    board.import([[dest1Coord, new PathCard.DestRockA()]]);
    expect(board.getCard(...dest1Coord)).toBeInstanceOf(PathCard.DestRockA);

    // 낙석 카드로 인해 고립된 카드가 생길 수 있음
    board.import([[[2, 0], new PathCard.Block4()]]);
    expect(board.getCard(2, 0)).toBeInstanceOf(PathCard.Block4);
  });
  ...
  describe("정상적이지 않은 보드 상태는 불러올 수 없다", () => {
    ...
    test("위치가 중복되는 카드가 있는 상태", () => {
      expect(() =>
        board.import([
          [[1, 0], new PathCard.Block4()],
          [[1, 0], new PathCard.Way4()],
        ]),
      ).toThrowError();
    });
    ...
    test("특별한 카드 위치를 덮어쓰는 잘못된 카드가 있는 상태", () => {
      expect(() =>
        board.import([[[0, 0], new PathCard.Block4()]]),
      ).toThrowError();
    });

    test("연결될 수 없는 카드가 연결되어있는 형태", () => {
      expect(() =>
        board.import([[[1, 0], new PathCard.Way2A()]]),
      ).toThrowError();
    });
  });
});
...
describe("카드 놓기", () => {
  test("이미 카드가 존재하는 위치에는 카드를 놓을 수 없다", () => {
    const board = new GameBoard();
    const card = new PathCard.Block4();
  ...
  test("시작점에서 길이 이어진 위치에만 카드를 놓을 수 있다", () => {
    ...
    const card = new PathCard.Way2A(true);
  ...
  test("인접한 모든 카드와 길이 연결되어있어야 카드를 놓을 수 있다", () => {
    ...
    const card2 = new PathCard.Way2A();
